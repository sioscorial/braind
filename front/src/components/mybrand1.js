...
function MyBrand() {
  ...
  useEffect(() => {
    const fetchBrandLikes = async () => {
      ...
      try {
        const response = await axios.get(`${apiUrl}/likes/brandAll`, {
  ...
  const handleCardClick = async (brandName) => {
  ...
  <HeaderBar>
    ...
    <HeaderContent>
      <Section>
        <Title color="#A0A0A0" onClick={handleMyBookmarkClick}>BOOKMARK</Title>
        <BottomBorder />
      </Section>
      <Section>
